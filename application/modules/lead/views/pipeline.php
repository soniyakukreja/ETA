...
<style>
  ...
  //   	var st = $('.checkstage').val();
  //   	var stage = $('#deal_stages').attr('data-stage');
  //   	alert(stage);
		// if(st==stage){
		// 	$("#map").focus();
					...
					<div class="input-group"><span class="input-group-addon"><i class="glyphicon glyphicon-th-list"></i></span>
						<select class="form-control" name="deal_stage" id="deal_stages">
							...
							<?php if (isset($stages)) {
								foreach ($stages as $value) { 
									if($value['last_stage']==0){ ?>
								 <option value="<?php echo $value['pstage_id']; ?>" class="checkstage"><?php echo $value['pstage_name']; ?></option>

								<?php } 

								if($value['last_stage']==1){ ?>

								 <option value="<?php echo $value['pstage_id']; ?>" id="assign" <?php if($this->userdata['dept_id']==11){ echo 'class="assign" stage-val="last"'; } ?>  ><?php echo $value['pstage_name']; ?></option>
								<?php }
									?>
							<?php 	}
							} ?>
