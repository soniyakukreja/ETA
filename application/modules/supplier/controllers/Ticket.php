...
class Ticket extends MX_Controller {
   ...
   public function __construct(){
    ...
    public function index()
    {
        ...
        $this->load->view('supplier/ticket');
    }
    ...
    public function ajax()
    {
        ...
        $datatables->edit('tic_id', function ($data) {
            return '<a href="'.site_url().'supplier/ticket/viewticket/'.encoding($data['tic_id']).'">View</a>';
        });
    ...
    public function addticket(){
        ...
        if(!empty($this->input->post()) && $this->input->is_ajax_request()){
            if($this->form_validation->run('consumer_addticket')){
                ...
                $eta_prod_manager = $this->generalmodel->getparticulardata('GROUP_CONCAT(user_id) AS ids ','user AS u','u.urole_id=1 AND dept_id=8 AND status=1','row_array');
                ...
                $data = array(
                    ...
                    'tic_users' => $eta_prod_manager['ids'], 
...
/*********** Add Note    ***********/
    ...
    {   
        ...
        $query = 'SELECT m.tic_activity_title,concat(b.firstname," ",b.lastname) as name,m.tic_activity_des,m.tic_activity_createdate FROM ticket_activity as m LEFT JOIN user as b ON m.tic_activity_createdby = b.user_id WHERE '.$where.' ORDER BY m.tic_activity_createdate DESC';
