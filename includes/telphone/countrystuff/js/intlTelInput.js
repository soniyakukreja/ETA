        ...
        function() {
            ...
            _createClass(Iti, [ {
                ...
                value: function _setFlag(countryCode) {
                    ...
                    this.telInput.setAttribute("data-id", this.selectedCountryData.dialCode+"-"+countryCode);
                    
                     var url      = window.location.href;
                 var uget = url.split("/");
                 if(uget[5] == 'lead_application' || uget[5] == 'completeapplication' || uget[5] == 'merchant_application' || uget[6] == 'boardingapplication' || uget[6] == 'uwcompleteapplication' || uget[6] == 'completeapplication' ){
                    if(this.telInput.name == 'phone' || this.telInput.name == 'alternativeph'){
                        
                        var ch = this.selectedCountryData.dialCode+"-"+countryCode; 
                        var name = this.telInput.name+"_codes";
                        summary_update_single(name,ch); 
                        
                    }
                 }
                 
              //MYCODE     
               /*Remoooooove for default settings*/ if(this.telInput.name!=''){
                  
                     var myid = this.telInput.name+'_codes';
                  
                    document.getElementById(myid).innerHTML = "<input type='hidden' name='"+this.telInput.name+"cods' value='"+this.selectedCountryData.dialCode+"-"+countryCode+"'>";
                  }
                  /*Remove for default settings by this im getting country and dial code in a hidden type*/
                  //MYCODE
                    if (this.options.separateDialCode) {
